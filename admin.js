...
function openNewDayModal() {
    ...
    AVAILABLE_TIME_SLOTS.forEach((time, index) => {
        ...
        select.className = 'time-slot-input time-select';
        select.style.cssText = 'flex: 1;';
        populateTimeSlotDropdown(select);
        select.value = time;
        
        const deleteBtn = document.createElement('button');
        deleteBtn.onclick = function() { removeTimeSlot(this); };
        deleteBtn.className = 'btn-danger';
        deleteBtn.style.cssText = 'padding: 0.5rem; font-size: 0.9rem;';
        deleteBtn.title = 'Verwijder tijdslot';
